...
If the last element in the list is a  dictionary with a key 'USD' and a value greater than 0,
yeild the list unchanged. If the last element of the list does not have a key 'USD' or
has such a key but with a value equal to 0, yeild the list with the last element removed."""

def remove_fbuy(dictionary_list):
	if dictionary_list[-1]['USD'] == 0:
...
for short in range(2, 100):
	for long in range(1, short):
		print(short, long, len(remove_fbuy(transaction_list_s({'USD' : 100.00 , 'BTC' : 0.0}, action_list(action(CLOSING_PRICE_LIST, short, long)), CLOSING_PRICE_LIST))))
